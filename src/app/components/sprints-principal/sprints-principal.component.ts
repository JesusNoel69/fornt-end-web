...
export class SprintsPrincipalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.projectService.getSelectedProject().subscribe((project) => {
      if (project) {
        // Llamamos al endpoint del backend para obtener los sprints del proyecto seleccionado
        this.sprintService.getSprintsByProjectId(project.Id).subscribe((sprints) => {
